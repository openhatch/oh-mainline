...
class Migration:
    ...
    def forwards(self, orm):
        ...
        assert not orm.Tag.objects.filter(tag_type__name='seeking')

        current_can_pitch_in_tag_texts = set()

        for tag in orm.Tag.objects.filter(tag_type__name='can_pitch_in'):
            current_can_pitch_in_tag_texts.add(tag.text)

        for text in current_can_pitch_in_tag_texts:
            # is this a dup?
            matches = orm.Tag.objects.filter(text=text, tag_type__name='can_pitch_in')
            if len(matches) > 1:
                # keep the first one
                winner = matches[0]
                # for each loser, make its Links point to the winner instead
                for loser in matches[1:]:
                    links = Link_Person_Tag.objects.filter(tag=loser)
                    for link in links:
                        link.tag_id = winner.id
                        link.save()
                    loser_obj = Tag.objects.get(id=loser.id)
                    loser_obj.delete() # the loser is gone!
    ...
    models = {
        ...
        'profile.citation': {
            ...
            'date_created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2010, 2, 23, 16, 3, 21, 746898)'}),
        ...
        'profile.dataimportattempt': {
            ...
            'date_created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2010, 2, 23, 16, 3, 21, 531657)'}),
        ...
        'profile.portfolioentry': {
            'date_created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2010, 2, 23, 16, 3, 21, 928244)'}),

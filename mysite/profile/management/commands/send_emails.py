...
class Command(BaseCommand):
    ...
    def get_projects_email_for(self, recipient):
        ...
        message_in_simpler_html = render_to_string(
            'email_re_projects.html', plain_context)
        message_in_html = render_to_string(
            'email_re_projects.html', rich_context)
    ...
    def get_context_for_email_to(self, recipient):
        ...
        for project in projects:
            ...
            display_these_contributors = [
                pfe.person for pfe in fellow_contributors_pfes]
            people_data['contributor_count'] = len(display_these_contributors)

            # Add wanna helpers
            # ---------------------
            notes = WannaHelperNote.objects.filter(
                within_range_for_wh, project=project)
            display_these_wannahelpers = [note.person for note in notes]
            people_data['wannahelper_count'] = len(display_these_wannahelpers)

            everybody = set(display_these_contributors +
            ...
            # Add to dictionary
            people_data[
                'display_these_contributors'] = display_these_contributors
            people_data[
                'display_these_wannahelpers'] = display_these_wannahelpers

            # Store dictionary inside tuple
            project2people.append((project, people_data))

        if not project2people:
            # If there's no news to report, signal this fact loudly to the
            # caller
            return None

...
class Person(models.Model):
    ...
    photo = models.ImageField(upload_to=lambda a, b: 'static/photos/profile-photos/' + generate_person_photo_path(a, b), default='')
    photo_thumbnail = models.ImageField(upload_to=lambda a, b: 'static/photos/profile-photos/' + generate_person_photo_path(a, b, suffix="-thumbnail"),
                                        default='', null=True)

    photo_thumbnail_30px_wide = models.ImageField(upload_to=lambda a, b: 'static/photos/profile-photos/' + generate_person_photo_path(a, b, suffix="-thumbnail-30px-wide"),
                                                  default='', null=True)

    photo_thumbnail_20px_wide = models.ImageField(upload_to=lambda a, b: 'static/photos/profile-photos/' + generate_person_photo_path(a, b, suffix="-thumbnail-20px-wide"),
                                                  default='', null=True)
    ...
    location_display_name = models.CharField(max_length=255, blank=True, default=DEFAULT_LOCATION, verbose_name='Location')
    latitude = models.FloatField(null=False, default=-37.3049962)
    longitude = models.FloatField(null=False, default=-12.6790445)
    email_me_re_projects = models.BooleanField(default=True, verbose_name='Email me periodically about activity in my projects')
    ...
    def __unicode__(self):
        return "username: %s, name: %s %s" % (self.user.username, self.user.first_name, self.user.last_name)
    ...
    def get_list_of_all_project_names(self):
        ...
        # the cache key above
        return list(self.get_published_portfolio_entries().values_list('project__name', flat=True).distinct())

    def get_display_names_of_nonarchived_projects(self):
        return list(self.get_nonarchived_published_portfolio_entries().values_list('project__display_name', flat=True).distinct())
    ...
    def _get_recommended_search_terms(self):
        ...
        terms.extend([pfe.project.name for pfe in self.get_published_portfolio_entries() if pfe.project.name and pfe.project.name.strip()])
    ...
    def get_tag_descriptions_for_keyword(self, keyword):
        ...
        return sorted([TagType.short_name2long_name[short] for short in [key for key in d if (keyword in d[key])]])
    ...
    def get_full_name_and_username(self):
        full_name_start = ("%s (" % self.get_full_name() if self.user.first_name or self.user.last_name else "")
...
class PublishedPortfolioEntries(models.Manager):
    ...
    def get_query_set(self):
        return super(PublishedPortfolioEntries, self).get_query_set().filter(is_deleted=False, is_published=True)
...
class Citation(models.Model):
    ...
    def save_and_check_for_duplicates(self):
        ...
        duplicates = [citation for citation in Citation.objects.filter(portfolio_entry=self.portfolio_entry) if (citation.pk != self.pk) and (citation.summary == self.summary)]
...
class Forwarder(models.Model):
    ...
    def garbage_collect():
        ...
        # them one.
        user_ids_that_need_forwarders = Person.objects.filter(contact_blurb__contains='$fwd').values_list('user_id', flat=True)

        user_ids_with_up_to_date_forwarders = Forwarder.objects.filter(user__id__in=user_ids_that_need_forwarders,

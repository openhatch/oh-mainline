...
class TrackerModelResource(tastypie.resources.ModelResource):
    ...
    class Meta:
        ...
        queryset = mysite.customs.models.TrackerModel.objects.select_subclasses().all()
    ...
    def get_object_list(self, request):
        """ Takes a request and returns a list of bug objects"""

        # Get all the objects from a tracker
        ...
        if tracker_type:
            ...
            query_for_corresponding_foreign_key = ~django.db.models.Q(**{tracker_type + 'trackermodel' + '__id': None,})
            try:
                all_objects = all_objects.filter(query_for_corresponding_foreign_key)
        ...
        if request.GET.get('just_stale', '').lower() == 'yes':
            ...
            for obj in all_objects:
                relevant_bugs = mysite.search.models.Bug.all_bugs.filter(tracker_id=obj.id)
                ...
                # is resolved.
                oldest_last_polled_data = relevant_bugs.aggregate(django.db.models.Min('last_polled'))
                oldest_last_polled = oldest_last_polled_data['last_polled__min']
                if oldest_last_polled and oldest_last_polled >= (datetime.datetime.utcnow() - datetime.timedelta(days=1)):

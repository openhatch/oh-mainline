...
def create_project_page_do(request):
    ...
    if project_name:
        ...
        if matches:
            ...
            # If project doesn't exists, create it, and go to project settings
            # page
            our_project, was_created = Project.objects.get_or_create(
                name=project_name)
...
def project(request, project__name=None):
    ...
    question_suggestion_response = request.GET.get(
        'question_suggestion_response', None)
...
def projects(request):
    ...
    return mysite.base.decorators.as_view(
        request, "project/projects.html", data,
...
def create_answer_do(request):
    ...
    try:
        answer.project = mysite.search.models.Project.objects.get(
            pk=request.POST['project__pk'])
        question = ProjectInvolvementQuestion.objects.get(
    ...
    # loltrolled--you dont have cookies, so we will throw away your data at
    # the last minute
...
def mark_contacted_do(request):
    ...
    project = django.shortcuts.get_object_or_404(
        Project, pk=request.POST.get('mark_contact-project'))
    for key in request.POST.keys():
        if key.endswith('checked'):
            person_pk = key[7:-8]
            # for each prefix, validate form
            mark_contacted_form = mysite.project.forms.MarkContactedForm(
                request.POST, prefix="helper-%s" % (person_pk))
            if mark_contacted_form.is_valid():
                ...
                whn = mysite.search.models.WannaHelperNote.objects.get(
                    person__id=person_id, project__id=project_id)
...
def wanna_help_do(request):
    ...
    if request.user.is_authenticated():
        ...
        wanna_help_queue = request.session.get(
            'projects_we_want_to_help_out', [])
...
def unlist_self_from_wanna_help_do(request):
    ...
    if wanna_help_form.is_valid():
        ...
        mysite.search.models.WannaHelperNote.remove_person_project(
            request.user.get_profile(), project)

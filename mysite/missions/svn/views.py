...
from mysite.missions.svn import forms, helpers
...
def resetrepo(request):
    ...
    helpers.SvnRepository(request.user.username).reset()
    helpers.unset_mission_completed(request.user.get_profile(), 'svn_checkout')
    helpers.unset_mission_completed(request.user.get_profile(), 'svn_diff')
    helpers.unset_mission_completed(request.user.get_profile(), 'svn_commit')
...
def diff_submit(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if form.is_valid():
                form.commit_diff()
                helpers.set_mission_completed(request.user.get_profile(), 'svn_diff')
...
def checkout_submit(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            helpers.set_mission_completed(request.user.get_profile(), 'svn_checkout')
            return HttpResponseRedirect(reverse('svn_checkout'))
...
class SvnBaseView(mysite.missions.base.views.MissionBaseView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if person:
            repo = helpers.SvnRepository(self.request.user.username)
            new_data.update({
                'repository_exists': repo.exists(),
                'svn_checkout_done': helpers.mission_completed(person, 'svn_checkout'),
                'svn_diff_done': helpers.mission_completed(person, 'svn_diff'),
                'svn_commit_done': helpers.mission_completed(person, 'svn_commit'),
            })
            if new_data['repository_exists']:
                new_data.update({
                        ...
                        'new_secret_word': helpers.SvnCommitMission.NEW_SECRET_WORD,
...
def commit_poll(request):
    return HttpResponse(simplejson.dumps(helpers.mission_completed(request.user.get_profile(), 'svn_commit')))

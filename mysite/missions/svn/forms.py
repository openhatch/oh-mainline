...
import os
from mysite.missions.base.helpers import get_mission_data_path
import patch
from mysite.missions.svn import helpers
...
class CheckoutForm(django.forms.Form):
    ...
    def clean_secret_word(self):
        cat_trunk = helpers.SvnRepository(self.username).cat('/trunk/' +
...
class DiffForm(django.forms.Form):
    ...
    def clean_diff(self):
        ...
        repo = helpers.SvnRepository(self.username)
        helpers.subproc_check_output(['svn', 'co', repo.file_trunk_url(), self.wcdir])
...
Thanks for reporting this, %s!''' % (self.FILE_TO_BE_PATCHED, self.username)
        helpers.subproc_check_output(['svn', 'commit', '-m', commit_message, '--username', 'mr_bad', self.wcdir])

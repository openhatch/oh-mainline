...
class DiffForm(django.forms.Form):
    diff = django.forms.CharField(
        error_messages={
            'required': 'No svn diff output was given.'},
    ...
    def __init__(
        ...
        self.username = username
        self.wcdir = wcdir
        if not wcdir is None:
            self.file_to_patch = os.path.join(wcdir, self.FILE_TO_BE_PATCHED)

    def clean_diff(self):
        """Validate the diff form.
        This function will be invoked by django.form.Forms.is_valid(),
        and will raise the exception ValidationError
        """
        ...
        if not self.the_patch._match_file_hunks(self.file_to_patch, self.the_patch.hunks[0]):
            raise ValidationError(
                'The patch will not apply correctly to the lastest revision.')

        # Check that the resulting file matches what is expected.
        self.new_content = ''.join(
            self.the_patch.patch_stream(
                open(
                    self.file_to_patch),

...
class TarMissionTests(TestCase):
    def test_good_tarball(self):
        view_helpers.TarMission.check_tarfile(open(make_testdata_filename('tar', 'good.tar.gz')).read())

    def test_bad_tarball_1(self):
        # should fail due to the tarball not containing a wrapper dir
        try:
            view_helpers.TarMission.check_tarfile(open(make_testdata_filename('tar', 'bad-1.tar.gz')).read())
    ...
    def test_bad_tarball_2(self):
        ...
        try:
            view_helpers.TarMission.check_tarfile(open(make_testdata_filename('tar', 'bad-2.tar')).read())
        except view_helpers.IncorrectTarFile, e:
    ...
    def test_bad_tarball_3(self):
        ...
        try:
            view_helpers.TarMission.check_tarfile(open(make_testdata_filename('tar', 'bad-3.tar.gz')).read())
        except view_helpers.IncorrectTarFile, e:
    ...
    def test_bad_tarball_4(self):
        ...
        try:
            view_helpers.TarMission.check_tarfile(open(make_testdata_filename('tar', 'bad-4.tar.gz')).read())
        except view_helpers.IncorrectTarFile, e:
    ...
    def test_bad_tarball_5(self):
        ...
        try:
            view_helpers.TarMission.check_tarfile(open(make_testdata_filename('tar', 'bad-5.tar.gz')).read())
        except view_helpers.IncorrectTarFile, e:
...
class UntarMissionTests(TestCase):
    ...
    def test_tarball_contains_file_we_want(self):
        tfile = tarfile.open(fileobj=StringIO(view_helpers.UntarMission.synthesize_tarball()), mode='r:gz')
        ...
        self.assertEqual(tfile.extractfile(file_we_want).read(), view_helpers.UntarMission.get_contents_we_want())
...
class UntarViewTests(TwillTests):
    ...
    def test_download_headers(self):
        ...
        self.assertEqual(response['Content-Disposition'], 'attachment; filename=%s' % view_helpers.UntarMission.TARBALL_NAME)
    ...
    def test_do_mission_correctly(self):
        ...
        contents_it_wants = tfile.extractfile(tfile.getmember(view_helpers.UntarMission.FILE_WE_WANT))
    ...
    def test_do_mission_incorrectly(self):
        ...
        bad_file.name = os.path.basename(view_helpers.UntarMission.FILE_WE_WANT)

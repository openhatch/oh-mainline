...
from mysite.missions.diffpatch import forms, helpers
...
def patchsingle_get_original_file(request):
    return make_download(open(helpers.PatchSingleFileMission.OLD_FILE).read(),
                         filename=os.path.basename(helpers.PatchSingleFileMission.OLD_FILE))

def patchsingle_get_patch(request):
    return make_download(helpers.PatchSingleFileMission.get_patch(),
                         filename=helpers.PatchSingleFileMission.PATCH_FILENAME)
...
def patchsingle_submit(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if form.cleaned_data['patched_file'].read() == open(helpers.PatchSingleFileMission.NEW_FILE).read():
...
def diffsingle_get_original_file(request):
    return make_download(open(helpers.DiffSingleFileMission.OLD_FILE).read(),
                         filename=os.path.basename(helpers.DiffSingleFileMission.OLD_FILE))
...
def diffsingle_submit(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                helpers.DiffSingleFileMission.validate_patch(form.cleaned_data['diff'])
...
def diffrecursive_get_original_tarball(request):
    return make_download(helpers.DiffRecursiveMission.synthesize_tarball(), filename=helpers.DiffRecursiveMission.TARBALL_NAME)
...
def diffrecursive_submit(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                helpers.DiffRecursiveMission.validate_patch(form.cleaned_data['diff'].read())
...
def patchrecursive_get_patch(request):
    return make_download(helpers.PatchRecursiveMission.get_patch(), filename=helpers.PatchRecursiveMission.BASE_NAME+'.patch')
